...
def send_to_zabbix(key, value):
    cmd = "zabbix_sender -z s.smarthome.net.ru -p 10051 -s raspberrypi.local -k %s -o %f" % (key, value)
    log_message(cmd)
...
slave_config = (
    ...
    },
    {
        ...
        'start':1,
        'registers':(
            {'key': 'smarthome.gasleak.uptime', 'multiplier': 1.0, 'offset': 0.0},
            {'key': 'smarthome.gasleak.gas_level', 'multiplier': 1.0, 'offset': 0.0}
        )
    },
    {
        'address':0x10,
        'start':1,
        'registers':(
            {'key': 'smarthome.floorheat.uptime', 'multiplier': 1.0, 'offset': 0.0},
            {'key': 'smarthome.floorheat.current_temp', 'multiplier': 0.0625, 'offset': 0.0},
            {'key': 'smarthome.floorheat.target_temp', 'multiplier': 1.0, 'offset': 0.0}
        )
    }
)
...
client = ModbusClient(method='rtu', port='/dev/ttyUSB0', timeout=0.3, baudrate=38400)
client.connect()
